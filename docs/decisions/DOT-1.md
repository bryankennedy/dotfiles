...
**Where open findings live.** Validating the skill's commands surfaced several untriaged findings. They are recorded in the **private** infrastructure repo at `docs/security-findings.md`, not here and not in `security-baseline.md` (which is for *accepted* findings, described rather than quoted). Individually most such findings are discoverable by anyone; collected into one ranked list they are a plan of attack, and that aggregation is the harm. A public repo is the wrong home for one.

**Why this file names no hostnames, accounts, or open weaknesses.** Writing the deny-list into the skill would have published, in a public repo, exactly the strings the skill exists to keep out of it — and would have labelled a previously ambiguous string as the production login account, a larger disclosure than the string itself. The skill therefore derives its terms at run time from `~/src/infrastructure/ansible/inventory/hosts.yml`. The private inventory was already the source of truth for the fleet; making it the source of truth for the audit's deny-list keeps the public repo free of the values.
